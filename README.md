...
*   **💻 Integrated Workspace IDE & Editor**: Collapsible File Explorer tree (recursive scans filtering `.git`, `node_modules`, etc.), monospaced code editing pane with scroll-synchronized line numbers, dynamic tab spacing, centered window title-bar path display, and keyboard event shortcut handlers (`Ctrl+S`, `Ctrl+B`, `Ctrl+K`).
*   **🎨 Multi-Theme System**: Instantly switch between the default **Industrial Slate** theme (professional dark slate-gray), the high-contrast **Minimalist Light** theme (clean light-gray design with a dark terminal), and the **Cyberpunk Neon** theme (glowing borders and canvas backgrounds), with client-side canvas loop suspension when inactive to save CPU cycles.
